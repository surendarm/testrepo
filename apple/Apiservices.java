...
public class Apiservices {
	...
	public Response servicesTSBData(@PathParam("makeid") String makeid,
			@PathParam("modelid") String modelid,
			@PathParam("yearid") String yearid,
			@PathParam("econfigid") String econfigid) throws JSONException {
	...
	@GET
	@Produces("application/json")
	public Response servicesPMData(@PathParam("makeid") String makeid,
			@PathParam("modelid") String modelid,
			@PathParam("yearid") String yearid,
			@PathParam("econfigid") String econfigid) throws JSONException {
		Connection conn = null;
		...
		try {
			...
			rs = st.executeQuery("SELECT DISTINCT pm_all.recno, pm_all_application.basevehicleiD, pm_all.qualifier, item.item,"
					...
					+ "WHERE (((pm_all_application.basevehicleiD)="
					+ basevehicleid + ")) order by frequency.freq");
	...
	@GET
	@Produces("application/json")
	public Response servicesModelDetails(@PathParam("makeid") String makeid,
			@PathParam("yearid") String yearid) throws JSONException {
		Connection conn = null;
		conn = connectToDatabaseOrDie();
		JSONObject res = new JSONObject();
		...
		try {
			Statement st = conn.createStatement();
			ResultSet rs1;
			rs1 = st.executeQuery("select distinct mo.modelid,mo.modelname,ma.makeid,ma.makename from basevehicle bas,make ma,model mo  where bas.makeid =ma.makeid   AND bas.modelid = mo.modelid AND bas.makeid = "
					+ makeid
					+ " AND bas.yearid ="
					+ yearid
	...
	@GET
	@Produces("application/json")
	public Response servicesMakeDetails(@PathParam("yearid") String yearid)
	...
	public Response videoLibrary() throws JSONException {
		...
		try {
			...
			rs = st.executeQuery("select * from video_library order by video_library_id desc");
			int i = 0;
			while (rs.next()) {
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("videoLibraryId",
						rs.getString("video_library_id"));
				jsonObject.put("videoTitle", rs.getString("video_title"));
				jsonObject.put("videoSource", rs.getString("video_source"));
				jsonObject.put("videoDescription",
	...
	public Response formName(@PathParam("u") int u) throws JSONException {
		...
		JSONObject jsonObject = new JSONObject();
		String result = null;
		String logo_image = "blank.png";
		int tempUserId = 0;
		try {
			Statement st = conn.createStatement();
			ResultSet mainrs, rs, innerrs;

			mainrs = st
					.executeQuery("select user_id from user_ifs where sess_user_id = "
							+ u);
			if (mainrs.next()) {
				do {
					// System.out.println("TEST ME");
					tempUserId = mainrs.getInt("user_id");
				} while (mainrs.next());

				innerrs = st
						.executeQuery("select image_file_name from user_settings_image where image_type='franchiseeimage' and user_id="
								+ tempUserId);
				while (innerrs.next()) {
					logo_image = innerrs.getString("image_file_name");
				}
				if (logo_image.equals("")) {
					logo_image = "blank.png";
				}

				rs = st.executeQuery("SELECT form_name, user_form_id from user_active_form where user_id = "
						+ tempUserId);
				if (rs.next()) {
					do {
						jsonObject.put("User ID", tempUserId);
						jsonObject.put("form_name", rs.getString("form_name"));
						jsonObject.put("form_id", rs.getInt("user_form_id"));
						jsonObject.put("logo_image", logo_image);
					} while (rs.next());
				} else {
	...
	@GET
	@Produces("application/json")
	public Response historyFieldsFillup(@PathParam("vin") String vin)
			throws JSONException {
		Connection conn = null;
		conn = connectToDatabaseOrDie();
		JSONObject jsonObject = new JSONObject();
		String result = null, tempContent = null, formType = null;
		int submissionID = 0;

		try {
			Statement st = conn.createStatement();
			ResultSet rs;
			rs = st.executeQuery("select form_submission_id from form_submission_profile_data where (profile_value = '"
					+ vin
					+ "' and not profile_value =  '') and profile_field = 'vin' order by form_submission_id desc limit 1");
			if (rs.next()) {
				submissionID = rs.getInt("form_submission_id");
				rs = st.executeQuery("select * from form_submission_index where form_submission_id = '"
						+ submissionID + "'");
				while (rs.next()) {
					formType = rs.getString("form_name");
				}
				rs = st.executeQuery("select * from form_submission_profile_data where form_submission_id = '"
						+ submissionID + "'");
				while (rs.next()) {
					tempContent = tempContent + " "
							+ rs.getString("profile_field") + "=\""
							+ rs.getString("profile_value") + "\"; ";

					jsonObject.put("form_submission_id", submissionID);
					jsonObject.put("form_type", formType);
					jsonObject.put("first_name",
							parseValueFromHeader(tempContent, "first_name"));
					jsonObject.put("last_name",
							parseValueFromHeader(tempContent, "last_name"));
					jsonObject.put("mobile_no",
							parseValueFromHeader(tempContent, "mobile_no"));
					jsonObject.put("vin",
							parseValueFromHeader(tempContent, "vin"));
					jsonObject.put("make",
							parseValueFromHeader(tempContent, "make"));
					jsonObject.put("model",
							parseValueFromHeader(tempContent, "model"));
					jsonObject.put("engine",
							parseValueFromHeader(tempContent, "engine"));
					jsonObject.put("year",
							parseValueFromHeader(tempContent, "year"));
					jsonObject.put("mileage",
							parseValueFromHeader(tempContent, "mileage"));
					jsonObject.put("Comments",
							parseValueFromHeader(tempContent, "Comments"));
					jsonObject.put("Date",
							parseValueFromHeader(tempContent, "Date"));
					jsonObject.put("tag",
							parseValueFromHeader(tempContent, "tag"));
					jsonObject.put("email",
							parseValueFromHeader(tempContent, "email"));
					jsonObject.put(
							"alternative_mobileno",
							parseValueFromHeader(tempContent,
									"alternative_mobileno"));
				}
	...
	@GET
	@Produces("application/json")
	public Response userServices(@PathParam("u") int u) throws JSONException {
		Connection conn = null;
		conn = connectToDatabaseOrDie();
		JSONObject jsonObject = new JSONObject();
		String result = null;
		...
		try {
			Statement st = conn.createStatement();
			ResultSet rs = st
					.executeQuery("select service_id from user_services where user_id = "
							+ u + " and service_active = 'true'");

			while (rs.next()) {
				rowValues.add(rs.getInt("service_id"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

		jsonObject.put("services", rowValues);
	...
	@GET
	@Produces("application/json")
	public Response servicesDescription(@PathParam("u") int u,
			@PathParam("id") int id) throws JSONException {
		Connection conn = null;
		conn = connectToDatabaseOrDie();
		JSONObject res = new JSONObject();
		String result = null;
		...
		try {
			Statement st = conn.createStatement();
			ResultSet rs;
			rs = st.executeQuery("select * from user_services where user_id = "
					+ u + " and service_id = " + id);
			while (rs.next()) {
				userServiceID = rs.getInt("user_service_id");
			}

			rs = st.executeQuery("select service_position_id, service_content from services_block_components where user_service_id ="
					+ userServiceID
					+ " and user_id ="
					+ u
					+ " order by service_position_id");
			int i = 0;
			while (rs.next()) {
				JSONObject jsonObject = new JSONObject();
		...
		try {
			...
			if (((int) Math.log10(userID) + 1) == 0) {
				...
				try {
					...
					Statement st = conn.createStatement();
					st.execute("Insert into form_submission_index (user_id, form_name, created_date) VALUES ("
							+ userID
							+ ",'"
							+ formName
							+ "','"
							...
							+ "')");
					ResultSet rs = st
							.executeQuery("select last_value from form_submission_index_form_submission_id_seq");
					while (rs.next()) {
						formSubmissionID = rs.getInt("last_value");
					}

					// st.execute("update form_submission_index set timer = '00:01' where form_submission_id = 1919;");
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response protest(String s) throws JSONException {
		...
		try {
			...
			if (((int) Math.log10(userID) + 1) == 0) {
				...
				try {
					...
					Statement st = conn.createStatement();
					st.execute("Insert into form_submission_index (user_id, form_name, created_date, timer) VALUES ("
							+ userID
							+ ",'"
							+ formName
							+ "','"
	...
	@GET
	@Produces("application/json")
	public Response formFields(@PathParam("fid") int fid) throws JSONException {
		Connection conn = null;
		conn = connectToDatabaseOrDie();
		JSONObject res = new JSONObject();
		String result = null;
		...
		try {
			Statement st = conn.createStatement();
			ResultSet rs = st
					.executeQuery("select distinct(c.component_id), c.component_name, sc.label, op.option_description, op.options_name, uc.position_id from components c left join user_forms_components uc on c.component_id = uc.component_id left join sub_components sc on c.component_id = sc.component_id left join sub_components_options sco on sc.sub_component_id = sco.sub_component_id left join options op on sco.option_id = op.option_id where uc.user_form_id = "
							+ fid + " order by uc.position_id");
			int i = 0;
			while (rs.next()) {
				JSONObject jsonObject = new JSONObject();
				jsonObject
				.put("component_name", rs.getString("component_name"));
				jsonObject.put("label", rs.getString("label"));
				jsonObject.put("option_description",
						rs.getString("option_description"));
				jsonObject.put("position_id", rs.getString("position_id"));
				jarray.put(i, jsonObject);
				i++;
			}
		} catch (Exception e) {
	...
	public Response camImageSubmission(FormDataMultiPart multiPart)
			throws JSONException {
		...
		for (String key : bodyList.keySet()) {
			if (key.equals("imgParam")) {
				...
				// String uploadedFileLocation = "/home/surendar/Downloads/" +
				// fileName;
				String uploadedFileLocation = "/home/innoppl/CVIFTabletApp/images/"
						+ fileName;
	...
	@Consumes("multipart/form-data")
	public Response imageSubmission(FormDataMultiPart multiPart)
			throws JSONException {
		...
		for (String key : bodyList.keySet()) {
			...
			if (!key.equals("submission_id") && !key.equals("user_id")
					&& !key.equals("sign")) {
				// System.out.println("Inside"+key);
				mvMap = multiPart.getField(key).getHeaders();
				headerList = mvMap.get("Content-Disposition");
	...
	private static String parseValueFromHeader(String header,
		...
		while (st.hasMoreTokens()) {
			...
			if (token.contains(parameterName + "=")) {
				...
				parameterValue = token.substring(quoteIndex + 1,
						token.length() - 1);
				// logger.finer("parameter " + parameterName + " is "+
				// parameterValue);
				break;
			}
		}
	...
	public Response fourHtml(@PathParam("u") int u) {
		...
		String ultimateContent = "", tempContent = "", css = "", profileContent = "", engine = "";
		String firstName = "", lastName = "", mobileNo = "", alternativeMobileNo = "", serviceType = "";
		String vin = "", make = "", model = "", year = "", mileage = "", comments = "", formDate = "", tag = "";
		...
		try {
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 14 and position_id <= 20");
			while (rs.next()) {
				b2Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id = 12");
			while (rs.next()) {
				bbSize++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 22 and position_id <= 26");
			while (rs.next()) {
				b3Size++;
			}
			i = 0;
			...
			i = 0;
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id in (32, 34, 36, 38)");
			while (rs.next()) {
				tempBlockFourZeroData[i][0] = rs.getString("component_name");
				tempBlockFourZeroData[i][1] = rs
						.getString("option_description");
				i++;
			}

			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 41 and position_id <= 44");
			while (rs.next()) {
				b41Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 48 and position_id <= 51");
			while (rs.next()) {
				b42Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 53 and position_id <= 56");
			while (rs.next()) {
				b43Size++;
			}
			i = 0;
			...
			i = 0;
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id in (33, 35, 37, 39)");
			while (rs.next()) {
			...
			i = 0;
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id in (46, 47)");
			while (rs.next()) {
				tempBlockPressureData[i][0] = rs.getString("component_name");
				tempBlockPressureData[i][1] = rs
						.getString("option_description");
				i++;
			}
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 62 and position_id <= 65");
			while (rs.next()) {
			...
			rs = st.executeQuery("select * from form_submission_profile_data where form_submission_id =  "
					+ submissionId);
			while (rs.next()) {
				tempContent = tempContent + " " + rs.getString("profile_field")
						+ "=\"" + rs.getString("profile_value") + "\"; ";
			}

			// Content for Services Block.

			rs = st.executeQuery("select profile_value from form_submission_profile_data where profile_field = 'service_type' and form_submission_id =  "
					+ submissionId);
			while (rs.next()) {
			...
			for (i = 0; i < tempBlockServiceData.length; i++) {
				...
				tempServiceFinalContent = tempBlockServiceData[i] + "<br>"
						+ tempServiceFinalContent;
			}
			...
			rs = st.executeQuery("select * from form_submission_image where form_submission_id =  "
					+ submissionId);
			while (rs.next()) {
				imageSize++;
			}

			rs = st.executeQuery("select * from form_submission_cam_image where form_submission_id =  "
					+ submissionId);
			while (rs.next()) {
				imageCamSize++;
			}
			...
			rs = st.executeQuery("select * from form_submission_sign_image where form_submission_id =  "
					+ submissionId);
			while (rs.next()) {
			...
			for (int m = 0; m < imageSize; m++) {
				imageContent = imageContent + "<img src='"
						+ Configuration.SERVICE_URL + "/images/"
						+ tempImageContent[m]
			...
			for (int m = 0; m < imageCamSize; m++) {
				imageContent = imageContent + "<div class='photo'><img src='"
						+ Configuration.SERVICE_URL + "/images/"
						+ tempImageCamContent[m]
								+ "' width='150px' height='100px'>"
								+ tempImageCamDesc[m] + "</div>";
			}

			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id =45");
			while (rs.next()) {
				tempBlockPressureImgData = rs.getString("option_description");
			}

			if (tempBlockPressureImgData.equals("1")) {
				tempBlockPressureImgContent = "<img src='"
						+ Configuration.SERVICE_URL
						+ "/images/system_files/red_t.png' class='tickimg' height='20' width='20'>";
			} else {
				tempBlockPressureImgContent = "<img src='"
						+ Configuration.SERVICE_URL
						+ "/images/system_files/red_ut.png' class='tickimg' height='20' width='20'>";
			}

			rs = st.executeQuery("select user_id from form_submission_index where form_submission_id = "
					+ submissionId);
			...
			rs = st.executeQuery("select image_file_name from user_settings_image where image_type='headerimageone' and user_id="
					+ tempUserId);
			while (rs.next()) {
			...
			rs = st.executeQuery("select image_file_name from user_settings_image where image_type='headerimagetwo' and user_id="
					+ tempUserId);
			while (rs.next()) {
			...
			rs = st.executeQuery("select image_file_name from user_settings_image where image_type='headerimagethree' and user_id="
					+ tempUserId);
			while (rs.next()) {
			...
			tag = parseValueFromHeader(tempContent, "tag");
			engine = parseValueFromHeader(tempContent, "engine");
			alternativeMobileNo = parseValueFromHeader(tempContent,
					"alternative_mobileno");
			...
			profileContent = "<div class='profile_title'><span class='profile_title'> PROFILE</span>"
					...
					+ "<li><label><b>First name </b></label> <label>"
					+ firstName
					+ "</label></li>"
					+ "<li><label><b>Last name </b></label> <label>"
					+ lastName
					+ "</label></li>"
					+ "<li><label><b>Moblie Phone </b></label> <label>"
					+ mobileNo
					+ "</label></li>"
					+ "<li><label><b>Alternate Phone </b></label> <label>"
					+ alternativeMobileNo
					+ "</label></li>"
					+ "<li><label><b>VIN </b></label> <label>"
					+ vin
					+ "</label></li>"
					+ "<li><label><b></b></label> <label></label></li>"
					+ "</ul></div><div class='profile-form-right'><ul>"
					+ "<li><label><b>Year </b></label> <label>"
					+ year
					+ "</label></li>"
					+ "<li><label><b>Make </b></label> <label>"
					+ make
					+ "</label></li>"
					+ "<li><label><b>Model </b></label> <label>"
					+ model
					+ "</label></li>"
					+ "<li><label><b>Engine </b></label> <label>"
					+ engine
					+ "</label></li>"
					+ "<li><label><b>Mileage </b></label> <label>"
					+ mileage
					+ "</label></li>"
					+ "<li><label><b>Tag Number </b></label> <label>"
					+ tag
					+ "</label></li>" + "</ul></div></div><br><br><br>";
			...
			for (j = 0; j < b1Size; j++) {
				...
				for (k = 0; k < 2; k++) {
					if (k == 0) {
						blockOneContent = blockOneContent + "<span>"
								+ tempBlockOneData[j][k] + "</span>";
					}
					...
					if (k == 1) {
						...
						if (tick.equals("1")) {
							blockOneContent = blockOneContent
									...
									+ Configuration.SERVICE_URL
									+ "/images/system_files/red_ut.png' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_ut.png'  class='tickimg' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/green_t.png' height='20' width='20'></div>";
							...
							blockOneContent = blockOneContent
									...
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_t.png'  class='tickimg' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/green_ut.png' height='20' width='20'></div>";
						} else if (tick.equals("3")) {
							blockOneContent = blockOneContent
									+ "<div style='float:right;'><img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/red_t.png' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_ut.png'  class='tickimg' height='20' width='20'>"
									+ "<img src='"
			...
			for (j = 0; j < bbSize; j++) {
				blockBatteryContent = blockBatteryContent
						+ "<div class='battery-box'><span>SEE ATTACHED PRINTOUT</span><br>";
				for (k = 0; k < 2; k++) {
					if (k == 1) {
						...
						if (tick.equals("1")) {
							blockBatteryContent = blockBatteryContent
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/green_t.png' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_ut.png'  class='tickimg' height='20' width='20'>"
									+ "<img src='"
							...
							blockBatteryContent = blockBatteryContent
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/green_ut.png' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_t.png'  class='tickimg' height='20' width='20'>"
									+ "<img src='"
			...
			blockBatteryContent = "<div class='box-1'><ul>"
					...
					+ blockBatteryContent
					+ "<div style='width:340px;'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/battery.png'></div></li></ul></div>";
			...
			for (j = 0; j < b2Size; j++) {
				...
				for (k = 0; k < 2; k++) {
					if (k == 0) {
						blockTwoContent = blockTwoContent + "<span>"
								+ tempBlockTwoData[j][k] + "</span>";
					}
					if (k == 1) {
			...
			for (j = 0; j < b3Size; j++) {
				...
				for (k = 0; k < 2; k++) {
					if (k == 0) {
						blockThreeContent = blockThreeContent + "<span>"
								+ tempBlockThreeData[j][k] + "</span>";
					}
					if (k == 1) {
			...
			for (j = 0; j < 2; j++) {
				if (j == 0) {
					blockFourZeroLeftContent = blockFourZeroLeftContent
							+ "<div class='box-conts-left'>";
				}
				if (j == 1) {
					blockFourZeroLeftContent = blockFourZeroLeftContent
							+ "<div class='box-conts-right'>";
				}

				for (k = 0; k < 2; k++) {
					if (k == 0) {
						blockFourZeroLeftContent = blockFourZeroLeftContent
								+ "<span>" + tempBlockFourZeroData[j][k]
										+ "</span>";
					}
					if (k == 1) {
				...
				blockFourZeroLeftContent = blockFourZeroLeftContent + " "
						+ tempBlockInchesData[j][1] + "/32\"</div>";
			}

			blockFourZeroLeftContent = "<li>" + blockFourZeroLeftContent
					+ "</li>";
			...
			blockFourZeroRightContent = "<li>" + blockFourZeroRightContent
					+ "</li>";

			for (j = 0; j < b41Size; j++) {
				if (j == 0) {
					blockFourOneContent = blockFourOneContent
							+ "<div class='tire-top'><span class='wear' style='font-size:13px;'><b><i>Wear Pattern/ Damage</i></b></span>";
				} else {
					blockFourOneContent = blockFourOneContent
							+ "<div class='tire-txt'>";
				}
				...
				for (k = 0; k < 2; k++) {
					if (k == 0) {
						blockFourOneContent = blockFourOneContent
								+ "<label style='padding-right:3px; vertical-align:middle; margin-top:5px;'>"
								+ tempBlockFourOneData[j][k] + "</label>";
					}
					if (k == 1) {
			...
			blockFourOneContent = "<div class='box-t'><img style='float:left' src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/wheel.png'>"
					+ blockFourOneContent
					+ "</div>";

			for (j = 0; j < b42Size; j++) {
				blockFourTwoContent = blockFourTwoContent
						+ "<div class='air-txt'>";
				for (k = 0; k < 2; k++) {
					if (k == 0)
						blockFourTwoContent = blockFourTwoContent + "<label>"
								+ tempBlockFourTwoData[j][k] + "</label>";
					if (k == 1)
						blockFourTwoContent = blockFourTwoContent
						+ "<input type='text' class='tire-input' value='"
						+ tempBlockFourTwoData[j][k] + "' readonly />";
			...
			blockFourTwoContent = "<li><div><span><b><i>Air Pressure</i></b></span></div>"
					+ "<div class='tire-top1'>"
					+ tempBlockPressureImgContent
					+ "</div><div><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/symbol.png'  width='40' height='45'></div>"
					...
					+ "<div class='air'>"
					+ "<div class='air-right'><input type='text' class='tire-biginput' value='"
					+ tempBlockPressureData[0][1]
							+ "' readonly />"
							+ "<p style='height:1px;'>&nbsp;</p><input type='text' class='tire-biginput' value='"
							+ tempBlockPressureData[1][1]
									+ "' readonly />"
									+ "</div>"
									+ blockFourTwoContent + "</div></li>";
				...
				for (k = 0; k < 2; k++) {
					if (k == 0) {
						if (tempBlockFourThreeData[j][k].equals("1")) {
							blockFourThreeContent = blockFourThreeContent
			...
			blockFiveContent = "<div class='box-5'><ul><li><div class='tit'><span class='tit-span'>BRAKES</span></div></li>"
					...
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/green_ut.png' height='20' width='20'><span style='padding-left:4px;'>Over 5 mm (Disk) or 2 mm (Drum)</span><br/>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/yellow_ut.png' height='20'width='20'><span style='padding-left:4px;'>3-5 mm (Disk) or 1.01-2 mm (Drum)</span><br/>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/red_ut.png' height='20' width='20'><span style='padding-left:4px;'>Less than 3 mm (Disk) or 1 mm (Drum)</span><br/>"
					+ "</div><div class='brake-3'><img style='float:left' src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/inspect_Brakes.png'></div>"
					+ "</div><div class='brake-2'>"
					+ blockFiveContent
					+ "</div><div style='background-color:#D4DAEE;float: left;text-align: center;width: 399px;'>"
					+ "<span style='font-weight:bold;font-size:15px;position:relative;bottom:20px;'>We feature</span><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/brands.png'><span style='font-weight:bold;font-size:15px;position:relative;bottom:20px;'>Brakes</span></div>"
					+ "</li></ul></div>";

			footContent1 = "<div class='comments'>COMMENTS </div><div style='float:left;'><p class='usercontent'>"
					+ comments
					+ "</p></div><div class='comments'>SERVICES </div><div style='float:left;'><p class='usercontent'>"
					+ tempServiceFinalContent + "</p></div>";
			...
			// "<div class='sign'><img src='http://localhost:8080/sampwebser/images/"
			// + signImage +"' width='150px' height='100px'></div></span>";
			footContent2 = footContent2 + "<div class='sign'><img src='"
					+ Configuration.SERVICE_URL + "/images/" + signImage
					+ "' width='150px' height='100px'></div>";

			footContent2 = footContent2
					+ "<div style='float:right; width:450px; text-align:right;'><div class='dat'>"
					+ formDate + "</div></div>";

			ultimateContent = "<html><head>"
					+ css
					+ "</head><body><div id='wrapper'><div class='header'>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					...
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/yellow_ut.png' height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>FUTURE ATTENTION</span></div>"
					+ "<div class='legend_right'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/red_ut.png' height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>IMMEDIATE ATTENTION</span></div>"
					+ "<div class='inn-lefts'>"
					+ blockOneContent
					+ blockBatteryContent
					+ blockTwoContent
					...
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/green_ut.png'  class='tickimg' height='20' width='20'><label style='font-style:italic; font-size:11px; font-weight:bold;'>7/32' greater</label>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/yellow_ut.png' class='tickimg' height='20' width='20'><label style='font-style:italic; font-size:11px; font-weight:bold;'>3/32' to 6/32'</label>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/red_ut.png'  class='tickimg' height='20' width='20'><label style='font-style:italic; font-size:11px; font-weight:bold;'>2/32' or less</label>"
					+ "</div></li>" + blockFourZeroLeftContent
					+ blockFourZeroRightContent
					+ "<li><ul><li><div class='box-tire'>"
					+ blockFourOneContent + interBlockFourContent
					+ "</div></li></ul></li>" + "</ul></div>"
					+ blockFiveContent + "</div></div>" + footContent1
					+ imageContent + footContent2
					+ "</div></div></body></html>";
	...
	public Response threeHtml(@PathParam("u") int u) {
		...
		try {
			...
			ResultSet rs = st
					.executeQuery("select * from form_submission_component_data where form_submission_id = "
							+ submissionId
							+ " and position_id >= 7 and position_id <= 13");
			while (rs.next()) {
				b1Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 16 and position_id <= 25");
			while (rs.next()) {
				b2Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 27 and position_id <= 29");
			while (rs.next()) {
				bbSize++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 31 and position_id <= 37");
			while (rs.next()) {
				b3Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 44 and position_id <= 51");
			while (rs.next()) {
				tempBlockFourZeroData[i][0] = rs.getString("component_name");
				tempBlockFourZeroData[i][1] = rs
						.getString("option_description");
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 61 and position_id <= 64");
			while (rs.next()) {
				b42Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 66 and position_id <= 69");
			while (rs.next()) {
				b43Size++;
			}
			i = 0;
			...
			i = 0;
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id in (59, 60)");
			while (rs.next()) {
				tempBlockPressureData[i][0] = rs.getString("component_name");
				tempBlockPressureData[i][1] = rs
						.getString("option_description");
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId + " and position_id =57");
			while (rs.next()) {
				tempBlockPressureImgData = rs.getString("option_description");
			}
			...
			blockBatteryContent = "<div class='box-1'><ul>"
					+ "<li><div class='tit'><span class='tit-span'>BATTERY</span></div></li><li>"
					+ blockBatteryContent
					+ "<div style='width:340px; text-align:right;'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/battery.png'></div></li></ul></div>";
			...
			blockFourTwoContent = "<li><div><span><b><i>Air Pressure</i></b></span></div>"
					+ "<div class='tire-top1'></div><div class='tire-top1'>"
					+ tempBlockPressureImgContent
			...
			ultimateContent = "<html><head>"
					...
					+ "<div class='inner-head'><h1>VEHICLE INSPECTION REPORT</h1></div>"
					+ profileContent
					+ "<div class='inn'><div class='legend_left'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/green_ut.png' height='20' width='20'>"
					+ "<span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>CHECKED AND OK</span></div><div class='legend_center'>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/yellow_ut.png' height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>MAY REQUIRE ATTENTION</span></div><div class='legend_right'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/red_ut.png' height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>REQUIRES IMMEDIATE ATTENTION</span></div>"
					+ "<div class='inn-lefts'>"
					+ blockOneContent
					+ blockTwoContent
					+ blockBatteryContent
					...
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/green_ut.png' height='20' width='20'><label style='font-style:italic; font-size:11px; font-weight:bold;'>7/32' greater</label>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/yellow_ut.png' height='20' width='20' class='tickimg' ><label style='font-style:italic; font-weight:bold; font-size:11px;'>3/32' to 6/32'</label>"
					+ "<img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/red_ut.png' height='20' width='20' class='tickimg' ><label style='font-style:italic; font-weight:bold;'>2/32' or less</label>"
					+ "</div></li>" + blockFourZeroLeftContent
					+ blockFourZeroRightContent
					+ "<li><ul><li><div class='box-tire'>"
					+ blockFourOneContent + interBlockFourContent
					+ "</div></li></ul></li>" + "</ul></div></div></div>"
					+ footContent1 + imageContent + footContent2
					+ "</div></div></body></html>";
	...
	public Response twoHTML(@PathParam("u") int u) {
		...
		String firstName = "", lastName = "", mobileNo = "", alternativeMobileNo = "", serviceType = "";
		String vin = "", make = "", model = "", year = "", mileage = "", comments = "", formDate = "", tag = "", engine = "";
		...
		try {
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 7 and position_id <= 21");
			while (rs.next()) {
				b1Size++;
			}
			i = 0;
			...
			rs = st.executeQuery("select * from form_submission_component_data where form_submission_id = "
					+ submissionId
					+ " and position_id >= 24 and position_id <= 37");
			while (rs.next()) {
				b2Size++;
			}
			i = 0;
					...
					if (k == 1) {
						...
						if (tick.equals("1")) {
							zeroContent = zeroContent
									...
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_ut.png' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/red_ut.png' height='20' width='20'></div>";
							...
							zeroContent = zeroContent
									...
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/yellow_t.png' height='20' width='20'>"
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/red_ut.png' height='20' width='20'></div>";
							...
							zeroContent = zeroContent
									...
									+ "<img src='"
									+ Configuration.SERVICE_URL
									+ "/images/system_files/red_t.png' height='20' width='20'></div>";
						} else {
							zeroContent = zeroContent
									+ "<div class='left_table_cell'>"
									+ "<img src='"
			...
			zeroContent = "<div class='left_table'><ul><li><div class='left_table_cell text_class' style=' color:#870101;'>ITEMS</div><div class='left_table_cell text_class' style=''><span class='left_table_cell text_class' style=' color:#870101; padding-left:10px;'>CONDITION</span></div></li>"
					+ zeroContent + "</ul></div>";
			...
			oneContent = "<div class='left_table'><ul><li><div class='left_table_cell text_class' style=' color:#870101;'>ITEMS</div><div class='left_table_cell text_class' style=''><span class='left_table_cell text_class' style=' color:#870101; padding-left:10px;'>CONDITION</span></div></li>"
					+ oneContent + "</ul></div>";
			...
			ultimateContent = "<html><head>"
					...
					+ "<div class='inner-head'><h1>VEHICLE INSPECTION REPORT</h1></div>"
					+ profileContent
					+ "<div class='clear'></div><div>&nbsp;</div><div class='legend'><div class='legend_left'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/green_ut.png'  height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>NO IMMEDIATE ATTENTION </span></div><div class='legend_center'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/yellow_ut.png' height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>MAY REQUIRE FUTURE ATTENTION</span></div> <div class='legend_right'><img src='"
					+ Configuration.SERVICE_URL
					+ "/images/system_files/red_ut.png' height='20' width='20'><span style='font-size:15px; padding-left:5px; padding-right:5px; font-weight:bold;'>IMMEDIATE ATTENDTION</span></div></div>"
	...
	public Response oneHtml(@PathParam("u") int u) {
		...
		try {
					...
					if (j < ctemp) {
						...
						if (k == 2) {
							if (tempComponentData[j][k].length() != 0) {
								zeroContent = zeroContent
										+ "<div style='float:left; width:300px;'><div style=' float:left;'><img src='"
										+ Configuration.SERVICE_URL
										+ "/images/system_files/grey_tick.png' class='tickimg'  height='20' width='20'><span style='padding-left:3px; padding-right:3px; font-size:14px;'>"
										+ tempComponentData[j][k]
												+ "</div></div>";
							} else {
			...
			zeroContent = "<div class='inn-left'><ul>" + zeroContent
					+ "</ul></div>";
			...
			for (int j = 0, l = ctemp; j < limit; j++, l++) {
				...
				for (int k = 0; k < 3; k++) {
					if (j != limit - 1) {
						if (k == 0)
							oneContent = oneContent + "<div class='inn-text'>"
									+ tempComponentData[l][k] + "</div>";
						if (k == 1)
							oneContent = oneContent + "<div class='inn-black'>"
									+ tempComponentData[l][k] + "</div>";
						if (k == 2) {
							if (tempComponentData[l][k].length() != 0) {
								oneContent = oneContent
			...
			oneContent = "<div class='inn-right'><ul>" + oneContent
					+ "</ul></div>";
			...
			ultimateContent = "<html><head>" + css
					+ "</head><body><div id='wrapper'><div class='header'>"
					+ "<img src='" + Configuration.SERVICE_URL
					+ "/images/settings_images/" + headerImageOne
					+ "' width='180' height='100'>" + "<img src='"
					+ Configuration.SERVICE_URL + "/images/settings_images/"
					+ headerImageTwo + "' width='600' height='100'>"
					+ "<img src='" + Configuration.SERVICE_URL
					+ "/images/settings_images/" + headerImageThree
					+ "' width='180' height='100'></div>"
					+ "<h1>VEHICLE INSPECTION REPORT</h1>" + profileContent
					+ "<div class='inn'>" + zeroContent + oneContent
					+ footContent1 + imageContent + footContent2
	...
	private Connection connectToDatabaseOrDie() {
		...
		try {
			...
			//String url = "jdbc:postgresql://127.0.0.1:5432/assistfeb";
